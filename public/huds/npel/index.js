const COLOR_CT = "rgba(224, 25, 73, 1.0)";
const COLOR_T = "rgba(2224, 25, 73, 1.0)";
const COLOR_NEW_CT = "rgba(255,255,255, 1.0)";
const COLOR_NEW_T = "rgba(255,255,255, 1.0)";
...
const COLOR_WHITE = "rgba(255, 255, 255, 1.0)";
...
const COLOR_WIN = "rgba(255, 255, 255, 1.0)";
...
function updateTopPanel() {
  //#region Team Name
    $("#map_team #team_map").text("| PICKED BY " + teams.left.name.toUpperCase());
  $("#left_team #main")
...
function updateStateFreezetime(phase, previously) {
  if (phase) {
    ...
    $("#round_timer_text").css("color", COLOR_WHITE);
...
function updateStateOver(phase, round, previously) {
  if (phase) {
    ...
    if (round.win_team == "CT") {
      if (teams.left.side == "ct") {
        // * CT alert on Left
        showAlertSlide("#left_team", COLOR_WIN, "WINS THE ROUND");
      } else {
        // * CT alert on Right
        showAlertSlide("#right_team", COLOR_WIN, "WINS THE ROUND");
      }
...
function fillPlayer(player, nr, side, observed, phase, previously) {
  ...
  if (slot >= 1 && slot <= 5) {
    ...
    $top.find("#player_alias_n").text(slot);
  } else {
    if (slot == 10) {
      $top.find("#player_alias_text").text(player.name);
      $top.find("#player_alias_n").text("0");
    } else {
      $top.find("#player_alias_text").text(player.name);
      $top.find("#player_alias_n").text(slot);
    }
  ...
  let gradient_single = "linear-gradient(to " + side.substr(8) + ", rgba(224, 25, 73, 0) " + (100 - stats.health) + "%, " + alt_health_color + " " + (100 - stats.health) + "%)";
  ...
  $bottom.find("#player_current_money_text").css("color", dead ? COLOR_WHITE_HALF : "#fff");
